...
| Deploy to a test Unraid box | `scripts/deploy-unraid.sh <ssh-host> [tag]`                 |

`deploy-unraid.sh` defaults to `tower latest` and recreates the container with the env,
mounts, and data it already has. `… <host> nightly` rides the prerelease channel, which can
apply migrations a later rollback to a stable tag cannot undo — Prisma only migrates forward,
so take a backup first.

Copy `.env.example` to `.env` for local work; it documents every variable the manager reads,
and the Unraid template sets the same ones in production. Never commit a real `.env`, key, or
API token.
